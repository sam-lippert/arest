...
\end{proposition}
\begin{proof}
A triggering event is the input $x$ and the current status a fact in $D$; advancing the live machine on that event is one AST step, with $D$ frozen throughout~\cite[Secs.\,14.3.1,\,14.6]{backus78}. The function $\mathit{machine}$ is an FFP expression over the event facts, its $\mathit{foldl}$ derived from Backus's $\mathit{while}$~\cite[Sec.\,11.2.4]{backus78}, evaluated within such a step, so it is a $\rho$-application (Proposition~\ref{prop:derive}) used for migration and audit; it alone orders by each event's occurrence timestamp $\tau$, while the live step takes events in arrival order. The two orders are Halpin's valid time and transaction time~\cite[Sec.\,10.3]{halpin08}: $\tau$ is a role of the event fact itself, ordinary data, so $\mathit{order}_\tau$ is derivable from $E$, whereas arrival order is the log's and is no fact of the domain.
\end{proof}
