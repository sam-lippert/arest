...
Halpin does not claim a modeller needs a tool. CSDP is a procedure a
person carries out, Neumont teaches Rmap by hand, and Sam has done it by
hand, so it is worth being exact about what this dependency actually is —
and it is deeper than "NORMA checks our work".

Every carrier is read back out of a NORMA model. The oracle parses FORML
sentences into NORMA model objects and then walks those objects to write
the bytes: `state:fts`, the schema itself, is
`myStore.ElementDirectory.FindElements<FactType>(true)`. NORMA is the
intermediate representation, not a validator bolted alongside one. So
there is no carrier that could be produced today without it, and
replacing it means replacing the model store rather than swapping a
checker.

The byte-level split, all three files generated by one run:

- `design-state` = `InputStateCells()` + `MappingStateCells(...)`.
  The first thirteen — fts, declared, nestings, otpops, derived,
  undelivered, rules, rings, readings, schemereadings, exclusions,
  setcmp, qualifiers — are read from the model THIS TOOL BUILT, so a
  mistake in them is the oracle's parse. The next sixteen — mapinputs,
  otmeta, ucs, djmands, deontics, factorder, refmodes, autoid, normamap,
  normacts, normaassim, normarels, normainfos, normauniq, normapaths,
  conorder — come from NORMA's own relational, abstraction and bridge
  assemblies, so a mistake in them is NORMA's mapping.
- `norma-answer` = NORMA's RMAP answer, the tables and column paths it
  derives, kept apart precisely so the canon's own mapping can be
  compared against it.
- `outcome` / `expectation` = the run's own surfaces, including
  `state:carriers`, one row per DEF above with its size and digest.

The USEFUL split is therefore semantic rather than by file. The parse leg
is ours: which sentence becomes which model element, and every arm in
Verifier.cs is that decision. The model leg is NORMA's: whether what we
built is well formed, what the intrinsic data types are, what Rmap makes
of it, and how it verbalizes. The read-back gate is the two legs checking
each other, which is why it is worth more than either alone.
