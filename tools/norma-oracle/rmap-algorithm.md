...
- **C.2.c No InformationTypeFormat (from an entity/structured value type mapped shallowly) ⇒ collapse the from-object-type's preferred identifier** (OMIFORM:1368-1461): recurse into each pid role's fact **in the identifier's role order** (`PreferredIdentifier.RoleCollection`; canon: the preferred `state:ucs` row, `cn:pidsort`), chaining `factTypePath`. Special handling: single-fact pid deeply mapped away (create a shallow "fake" mapping, OMIFORM:1411-1419); cyclic deep collapse (subtype identified by supertype + objectified 1-1) is broken by forwarding a shallow mapping using `parentConceptTypeHasDeepAway` (OMIFORM:1420-1457).
...
> *Canon encoding* (`rmap:sepnames`): `rmap:gmi` flag slot 1 is is-subtype and slot 3 is objectification-implied, so the `Separate` set is the deep assimilations with NEITHER flag set — pascal-cased. The D.3.4/D.3.5 consequence is `rmap:seprefs` <assimilated, assimilator, deep fact, IsPreferredForTarget>: `rmap:colpathsP` drops what the walk gathered beneath a separated assimilation (D.3.4, nothing on the assimilator's side) and gives the assimilated concept type's own table a relation to its assimilator over the deep fact (D.3.5: the assimilator's preferred-identifier columns, a non-primary uniqueness, a reference constraint), spelled `assim` assimilator assimilated at the `norma:colpaths` witness, the child NORMA links the column to. Note this is the D.2 clause the discipline block names as a past correlate: reproduce the RULE (both flags, either one absorbing) and not merely the answer for the models at hand.
...
**D.7.4 Column name** `GenerateColumnName` (NAMEGEN:523-845): reconstructs the column's `ColumnPathStep` chain from the `ColumnHasConceptTypeChild` path (`GetColumnPath`) and walks it. Value-type value column with empty path ⇒ `ResourceStrings.NameGenerationValueTypeValueColumn`. Empty final name ⇒ `"COLUMN"` (phase 0). Exact transcription (all read whole 2026-07-22/23):

**D.7.4a ColumnPathStep construction** `GetColumnPath` (NAMEGEN:1160-1520) — a TABLE-level pass, columns processed IN COLUMN ORDER (decoration mutates earlier columns' steps; the per-column result is cached per table):
- Flags (NAMEGEN:895-963): ForwardAssimilation, ReverseAssimilation, IsIdentifier, PassedIdentifier, RequiresDecoration, ObjectifiedFactType, AssimilationIsSubtype, DeclinedAssimilation, NonPreferredSubtype, AssimilationTowardsSubtype, PositiveUnary, NegativeUnary, InvertibleUnary, AbsorptionIndicator.
- Cross-cut dictionary `objectTypeToSteps` seeded with the table's primary AND `TableIsAlsoForConceptType` object types as roleless steps (1183-1196) — paths looping back into the table's own types always diverge ⇒ decorate.
- Per child link, per PATH FACT TYPE (`ConceptTypeChildHasPathFactType`, = one recorded step of ours): `FromRole = FactTypeMapsTowardsRole` (the certified stage-1 towards decision); flags seed = PassedIdentifier latch + AbsorptionIndicator from the child link (1282-1285).
- Assimilation children (1224-1268, 1292-1408): direction from `comingFromConceptType` vs `assimilation.Parent` (else towardsSubtype); Forward iff absorption choice = Absorb, else Reverse; once a tail is moving in a direction KEEP IT; a forward subtype tail meeting a non-Absorb subtype assim mid-chain = forwardToReverseTransition ⇒ DeclinedAssimilation, process as plain fact. CONSECUTIVE SUBTYPE assims COLLAPSE into the single tail node (`continue` @1354) accumulating AbsorptionIndicator/NonPreferredSubtype (secondary = RefersToSubtype ∧ ¬IsPreferredForTarget). Non-subtype assim whose fact is implied by an objectification nesting the target ⇒ hold `assimilationObjectification`; an ABSORPTION-INDICATOR unary objectification instead re-processes as the direct unary fact (1307-1316, ignoreObjectification).
- Objectified-nest splice (1422-1441): next fact implied by the held objectification with a proxy opposite ⇒ ObjectifiedFactType; replace with the ORIGINAL unobjectified fact's roles; if the tail step IS the nesting type ("trivial path leading in") REMOVE the subtype node entirely and continue.
- Plain fact step (1455-1485): opposite role (proxy-resolved unless ignoreObjectification) gives the target; NO opposite ⇒ unary: Positive/NegativeUnary by `UnaryPattern == Negation`, InvertibleUnary iff `InverseConceptTypeChild` present. **IsIdentifier** iff the opposite (or unary) role ∈ the target player's `PreferredIdentifier.RoleCollection`; sets the passedIdentifier latch.
- **RequiresDecoration** (`ProcessTailNode` 1521-1605): when a completed step's TargetObjectType already has cross-cut entries with a DIFFERENT FromRole, decorate — the current step, the cross-cut steps, and/or (when the divergence lies in non-identifier ancestry, compared via `GetPreviousNonIdentifierNode`/`CoreEquals`) the ancestor non-identifier cross-cuts (`DecorateCrossCutNonIdentifiers`).

**D.7.4b The name walk** (NAMEGEN:558-838), per step, in order:
- ForwardAssimilation: skip name if the last step consumed the next node or used an explicit role name and the previous target == this StartingObjectType; else emit `"is"` iff (AssimilationIsSubtype ∧ AbsorptionIndicator), then `SeparateObjectTypeParts(ResolvedSupertype)` (571-584).
- ReverseAssimilation: NO name parts; treatNextIdentifierAsFirstStep = firstPass (a later IsIdentifier step re-enters firstPass naming); tail-position ValueType-with-own-table exception emits the supertype parts (585-596).
- Plain fact: emit iff RequiresDecoration ∨ incorporateNextAssimilation (next step = ForwardAssimilation starting at this target) ∨ (last node ∧ (¬IsIdentifier ∨ ¬(consumed|explicitRole))) (599-604). **Back-up-one-step** (607-629): last node, IsIdentifier, ¬decorate, previous step plain, previous resolved supertype HAS a refmode pattern ⇒ re-walk the PREVIOUS step (picks up role/hyphen parts instead of bare predicate text).
- Emission priority per emitting step (630-824): (1) front-text-free matching READING — only when (decorate ∧ phase 1) or (unary ∧ no explicit role name); unpaired nameless unary negation borrows the positive unary + literal `"not"` (649-665); reading text splits around the far-role placeholder, target resolved via `ResolveObjectTypeName` between the halves; (2) explicit FAR ROLE NAME (733-737); (3) HYPHEN-BOUND format string from the matching reading order around `ResolveObjectTypeName` (740-824); target resolution inside (1)/(3): incorporateNextAssimilation consumes the NEXT step's resolved type; last-node emission resolves `(firstPass ? PrimaryIdentifier : ReferenceToEntityType)` against the previous resolved super/object type (711-726, 809-817).
- Loop bookkeeping: previous(Previous)Resolved(Super|Object)Type roll forward each step (Reverse steps roll `ResolvedSupertypeVerifyPreferred`); firstPass clears after step 1.

**D.7.4c Refmode resolution** `ResolveObjectTypeName` (REFMODE:2664-2800): entity WITH a refmode pattern dispatches on the naming choice (ModelDefault ⇒ relational defaults per refModeType × use — the PrimaryIdentifier/ReferenceToEntityType distinction of D.7.4b): EntityTypeName ⇒ `SeparateObjectTypeParts(entity)`; ValueTypeName ⇒ abbreviation else refmode parts; ReferenceModeName ⇒ the mode name; CustomFormat ⇒ `{Entity}/{Value}/{RefMode}` splice. Passed valueType ≠ the actual pid role player ⇒ `Separate(prefer ? entity : value)` and out. `SeparateObjectTypeParts` (REFMODE:2922-2954): abbreviation → objectified-type-with-default-name: separate the DEFAULT READING (`SeparateReadingParts` 2962-2982: before-text with `"- "`/`" -"` → `" "`, role players recurse) → valuetype-with-refmode: refmode parts → native name.

**D.7.4d Station vacuities (proven against the oracle):** the FORML parser never creates negation unaries (`UnaryValuePattern` appears once in Verifier.cs, a test) ⇒ NegativeUnary/InvertibleUnary/the `"not"` borrow are constants; no abbreviations/RecognizedPhrase elements are declared ⇒ `ResolveRecognizedPhrases` and `GetAbbreviatedName` are identity; explicit role names are absent from FORML models ⇒ arm (2) vacuous (re-verify at serialization).
