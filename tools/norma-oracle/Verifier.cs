...
namespace Arest.NormaOracle
{
	internal sealed class Verifier
	{
		...
		private List<string> myDerivedPairs = new List<string>();
		...
		public List<string> ReadBackDerivationRules()
		{
			...
			myReadBackMismatches = mismatches + unmatchedTexts;
			return log;
		}
		...
		// THE RUN'S OUTCOME IS A SURFACE OF THE CARRIER, not a text the test project
		// scrapes: the built rule heads with multiplicity, sorted so two runs compare
		// as sequences; the blocking-error total; the read-back count (paths that
		// read as no rule of their head, plus built rules no path reads). The same
		// three under expect: names are the recorded expectation, a carrier of its
		// own spliced beside a later run's design-state, and law:regress compares
		// the two in canon; the hosts answer `regress`.
		private static int myBlockingErrors;   // set by the static error writer; one verifier per process
		private int myReadBackMismatches;
		private string OutcomeCells(string prefix, List<string> built)
		{
			var heads = new List<string>();
			foreach (string l in built)
			{
				// a built rule is `head := recipe`; the list's closing summary line is not one
				int at = l.IndexOf(" := ", StringComparison.Ordinal);
				if (at < 0) continue;
				heads.Add(IAtom(l.Substring(0, at).Trim()));
			}
		...
		{
			WriteCarrier(path, "(\n\"THE RECORDED EXPECTATION in INTERSECTION SOURCE (generated by norma-oracle; regenerate, never edit): an accepted run's state:built, state:errors and state:readback under expect: names. Spliced beside a later run's design-state as the carrier `expected`, so law:regress can hold the run to it.\",\n\n"
				+ OutcomeCells("expect", built).TrimEnd().TrimEnd(',') + "\n)\n");
		}
		// THE BUILD SURFACE AS FACTS, not as a census line. This tool already
		// computes which fact types carry a derivation mode, which of them a
		// recipe delivers, and why an arm declined the rest -- and then PRINTS
		// them. A printed census dies with the run: "which fact types are inert
		// on auto.dev" has been answered by re-running the oracle and grepping,
		// once per session, for as long as the question has existed.
		//
		// These are the same facts written in the vocabulary of
		// apps/arest-dev/readings/build-surface.md, which is the metamodel's own
		// (`Derivation Rule produces Fact Type`, `Fact Type has Derivation
		// Mode`) plus the two that model IS new for. A corpus that reads this
		// file gets a population, so `Fact Type is inert` stops answering empty
		// and the deontic `It is forbidden that some Fact Type is inert` has
		// something to be violated by.
		//
		// FORML, NOT A CARRIER, and that is the point rather than a detail. A
		// carrier is a DEF spliced into a store; these have to enter a store
		// THROUGH a model, so they are instance facts a corpus reads like any
		// other reading. The oracle has not written readings before.
		//
		// The Derivation Mode values are the MODEL's four -- Halpin's markers as
		// single values, per core.md:347 -- not state:derived's row encoding,
		// which emits `semi` and `stored` separately for one head. Reading the
		// carrier's shape as the model's shape is the error build-surface.md's
		// own CSDP note records three instances of; this is where it would land
		// a fourth if the mapping were skipped.
		//
		...
		// the derivation surface: every derivation-marked name with its mode --
		// 'full' (*), 'stored' (**), 'semi' (+), 'subtype' (Fig 13.29 form) --
		// which state:derived carries and the build-surface fact types read
		private List<string> ComputeDerivedPairs()
		{
		...
		// each derivation-marked fact type's Derivation Mode as the metamodel
		// names them (core.md: fully-derived, derived-and-stored, semi-derived,
		// semi-derived-and-stored); a subtype definition carries none
		private Dictionary<string, string> DerivationModes()
		{
			var modes = new Dictionary<string, SortedSet<string>>(StringComparer.Ordinal);
			foreach (string pair in ComputeDerivedPairs())
			{
				Match pm = Regex.Match(pair, "^S2\\(A\\(\"(.+?)\"\\), A\\(\"(.+?)\"\\)\\)$");
				if (!pm.Success) continue;
				SortedSet<string> set;
				if (!modes.TryGetValue(pm.Groups[1].Value, out set)) modes[pm.Groups[1].Value] = set = new SortedSet<string>(StringComparer.Ordinal);
				set.Add(pm.Groups[2].Value);
			}
			var result = new Dictionary<string, string>(StringComparer.Ordinal);
			foreach (var kv in modes)
			{
				string mode = kv.Value.Contains("subtype") ? null
					: kv.Value.Contains("semi") && kv.Value.Contains("stored") ? "semi-derived-and-stored"
					: kv.Value.Contains("semi") ? "semi-derived"
					: kv.Value.Contains("stored") ? "derived-and-stored"
					: kv.Value.Contains("full") ? "fully-derived" : null;
				if (mode != null) result[kv.Key] = mode;
			}
			return result;
		}
		...
		public void WriteBuildFacts(string path)
		{
			...
			foreach (string r in myRuleRecipes)
			{
				...
				int b = a < 0 ? -1 : r.IndexOf('"', a + 3);
				if (b > a) recipeHeads.Add(r.Substring(a + 3, b - a - 3));
			}
			...
			var sb = new System.Text.StringBuilder();
            sb.Append("# Build surface, measured\n\n");
            sb.Append("# GENERATED by norma-oracle; regenerate, never edit. Instance facts in the\n");
            sb.Append("# vocabulary of apps/arest-dev/readings/build-surface.md, so a corpus that\n");
            sb.Append("# reads this file can answer `Fact Type is inert` instead of a log line.\n\n");
            sb.Append("## Instance Facts\n\n");
			foreach (var kv in DerivationModes().OrderBy(x => x.Key, StringComparer.Ordinal))
			{
				string mode = kv.Value;
				sb.Append("Fact Type '").Append(kv.Key).Append("' has Derivation Mode '").Append(mode).Append("'.\n");
				sb.Append("Derivation Rule 'r:").Append(kv.Key).Append("' produces Fact Type '").Append(kv.Key).Append("'.\n");
				string why;
				if (recipeHeads.Contains(kv.Key))
		...
		public void WriteDesignState(string path, string extraCells = null)
		{
			...
			// AND THE DERIVATION MODE LANDS IN ITS OWN FACT TYPE (arest #94, #115).
			// The modes this tool computes went to build-facts.md in arest-dev's
			// vocabulary and to the log, while the metamodel declares `Fact Type has
			// Derivation Mode` (core.md) and every store carried it empty, so the
			// served resource and the MCP answered [] (2026-09-10). An explicit row
			// in the readings sets a mode and wins. The rule sentences are NOT
			// written as Derivation Rule instances here: the metamodel makes every
			// Derivation Rule carry some Antecedent Clause and some Role Projection
			// (core.md 1279, 1391), and the Roles a projection produces are minted
			// by canon when it reflects the meta-types at boot, not by this tool; a
			// bare rule row is an alethic violation the writable law refuses, which
			// it did (38 rules, 76 violations, the base report). Those populations
			// follow when the rule structure is written against the reflected roles.
			{
				FactIndexEntry modeE = null;
				foreach (FactIndexEntry e in myFactIndex)
					if (e.Fact != null && !e.Fact.IsDeleted && e.Fact.Name == "FactTypeHasDerivationMode") { modeE = e; break; }
				if (modeE != null)
				{
					var haveMode = new HashSet<string>(StringComparer.Ordinal);
					foreach (var row in modeE.Rows) if (row.Count == 2) haveMode.Add(row[0]);
					foreach (var kv in DerivationModes().OrderBy(x => x.Key, StringComparer.Ordinal))
					{
						if (haveMode.Contains(kv.Key)) continue;
						modeE.Rows.Add(new List<string> { kv.Key, kv.Value });
						modeE.RowKinds.Add(new List<string> { "Fact Type", "" });
					}
				}
			}
			...
			var derivedPairs = ComputeDerivedPairs();
			// kept for WriteBuildFacts, which turns these into instance facts
			myDerivedPairs = derivedPairs;
