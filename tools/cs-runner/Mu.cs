...
public static partial class Arest
{
        ...
        new Dictionary<string, Func<object, object>>(StringComparer.Ordinal)
    {
        ...
        { "ntoa", x => { if (!(x is int)) throw new InvalidOperationException("ntoa on non-number");
            return ((int)x).ToString(System.Globalization.CultureInfo.InvariantCulture); } },
        { "quote_str", x => "\"" + ((string)x).Replace("\\", "\\\\").Replace("\"", "\\\"").Replace("\n", "\\n").Replace("\r", "\\r") + "\"" },
