...
// L2 table IS fetch — and application IS fetch, chained: a relation cell
// holds its rows curried (rmap:nest), so fetching a key value inside the
// cell answers that key's image, one parameter at a time
...
try {
  ...
  for (const d of projDescs.filter(d => d[4].length > 0)) {
    ...
    for (const k of [...new Set(d[4].map(r => r[0]))]) {
      ...
      if (setJson(image) !== setJson(want)) {
        l2 = false;
        if (!l2note) l2note = " (chain diverged at " + d[0] + " / " + k + ")";
      }
      chains++;
    }
  }
  if (!l2note) l2note = " incl. " + chains + " curried applications";
} catch (e) { l2 = false; l2note = " (" + e.message + ")"; }
laws.push(["L2 fetch = restrict-project: table access through ast:Fetch" + l2note, l2]);
// L5 currying agreement: the two RMAPs must agree row for row — rmap:unnest
// of every emitted relation cell reproduces exactly the population rows the
// flat (NORMA) presentation asserts; nest and unnest are inverses
...
try {
  for (const d of projDescs) {
    ...
    if (setJson(got) !== setJson(d[4])) {
      l5 = false;
      if (!l5note) l5note = " (diverged at " + d[0] + ")";
    }
  }
} catch (e) { l5 = false; l5note = " (" + e.message + ")"; }
laws.push(["L5 currying: unnest of every relation cell = its rows (" +
  l5checked + " cells)" + l5note, l5]);
...
// no value-domain shunt: a NORMA table the canon cannot account for is a
// MISMATCH, never a tolerated class — a roleless value type maps to a
// degree-1 relation (the active domain, derived), and hiding it here is
// how four orphan declarations survived until 2026-07-16
const factTables = [], absorbingTables = [];
for (const t of normaTables) {
  if (byName.has(norm(t.name))) factTables.push(t.name);
...
fs.writeFileSync(path.join(__dirname, "checker-answer"),
  '(\n"THE CHECKER ANSWER in INTERSECTION SOURCE (generated by js-runner; regenerate, never edit). checker:store — the store rmap answered: entity cells with wide rows, relation cells with their populations CURRIED (rmap:nest — one store level per role, unnest for the flat view).",\n\nDEF("checker:store", ' +
  emit(store) + ")\n)\n");
