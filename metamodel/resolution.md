...
<!-- AND THE ONE THAT REMOVES (2026-09-21). The name collision the paragraph
     ...
     side of main:api0 needed no new plumbing: main:retract_step, main:retract
     and main:retract_rows already answer http:method_kinds' `retraction` kind
     (case:retract-removes, case:retract-absent,
     case:a-retraction-refuses-where-a-replacement-commits in
     engine/shared/scenarios.canon), unreachable from the MCP only because the
     verb name was taken.

     THE TWO ROWS THAT SERVE IT ARE HELD, beside create's and replace's above;
     `Operation 'retract' is overridable` was already asserted (the catalog
     above), so the accepts/yields pair was the whole of what was missing --
     ...
     row is in the population; retract with the same row answers 201
     committed and the row is gone; retract again answers 201 and changes
     nothing, case:retract-absent's no-op; a row carrying the id alone is
...
<!-- llm:validate_judge NAMED HERE ONLY IN PROSE UNTIL NOW (#122 item 7,
     ...
     `does fill` (registered; arest:13208, "a registered row naming a
     capability no host provides is a FALSE entry in Cor 6 enumerable
     boundary"). Declared registrable here and left off the `is
     registered` list below -- the same honest-until-filled state
     synthesize and validate are in. Accepts and yields reuse validate's
     own two Type Expressions verbatim rather than invent a new shape:
     support's state-law-wiring.md already has the target populated --
